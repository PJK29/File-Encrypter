...
stage('Test') {
    steps {
        ...
        # Download JUnit jar if not already present or if corrupted
        if [ ! -f junit-platform-console-standalone.jar ] || ! unzip -tq junit-platform-console-standalone.jar >/dev/null 2>&1; then
            echo "Downloading JUnit..."
            curl -fL -o junit-platform-console-standalone.jar https://repo1.maven.org/maven2/org/junit/platform/junit-platform-console-standalone/1.10.0/junit-platform-console-standalone-1.10.0.jar
        fi
